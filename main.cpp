...
template <typename T> struct V2 {
  ...
  template <typename O> V2<O> cast() const { return V2<O>(O(x), O(y)); }
};

template <typename T> V2<T> operator+(V2<T> a, V2<T> b) { return a.add(b); }

template <typename T> V2<T> operator-(V2<T> a, V2<T> b) { return a.sub(b); }

template <typename T> V2<T> operator-(V2<T> a) { return V2<T>(-a.x, -a.y); }

template <typename T> V2<T> operator*(float f, V2<T> a) { return a.scale(f); }

template <typename T> V2<T> operator*(V2<T> a, float f) { return a.scale(f); }

template <typename T> V2<T> operator/(V2<T> a, float f) {
...
// catmull rom spline
template <typename T> T catmullrom(T p0, T p1, T p2, T p3, float t) {
  return 0.5 *
...
void draw_pen_strokes_cr(cairo_t *cr) {
  ...
  for (int xix = startx / HASH_CELL_SZ - 1; xix <= endx / HASH_CELL_SZ; ++xix) {
    for (int yix = starty / HASH_CELL_SZ - 1; yix < endy; ++yix) {
      ...
      for (int cix : bucket) {
        ...
        V2<float> cr1 =
            g_renderstate.zoom * (c.posStart - g_renderstate.pan).cast<float>();
        V2<float> cr2 =
            g_renderstate.zoom * (c.posEnd - g_renderstate.pan).cast<float>();
...
void draw_palette(SDL_Renderer *renderer) {
  ...
  // draw eraser.
  {
    ...
    rect.h =
        (g_colorstate.is_eraser ? SELECTED_PALETTE_HEIGHT : PALETTE_HEIGHT());
...
void cairo_setup_surface() {
  ...
  g_cr_surface = cairo_image_surface_create_for_data(
      (unsigned char *)g_sdl_surface->pixels, CAIRO_FORMAT_RGB24, SCREEN_WIDTH,
...
void handle_packet(int p) {
  ...
  if (g_colorstate.is_eraser &&
    ...
    for (int xix = startx / HASH_CELL_SZ - 1; xix <= endx / HASH_CELL_SZ + 1;
         ++xix) {
      for (int yix = starty / HASH_CELL_SZ - 1; yix <= endy / HASH_CELL_SZ + 1;
        ...
        for (int cix : bucket) {
          ...
          const V2<int> delta = g_renderstate.pan + g_penstate - c.pos();
          // eraser has some radius without pressing.
          // With pressing, becomes bigger.
          if (delta.lensq() <=
              g_colorstate.eraser_radius * g_colorstate.eraser_radius) {
...
int main() {
  ...
  while (!g_quit) {
    ...
    if (timeToNextFrameMs > elapsedMS) {
      ...
      printf("%20s | elapsed time: %4.2f | sleeping for: %4.2f | time to "
             "next frame: %4.2f\n",
